...
"use strict";
/*-------------------------------------------
	Dynamically  plugin scripts
---------------------------------------------*/
//
// Dynamically  Fullcalendar Plugin Script
// homepage: http://arshaw.com/fullcalendar
// require moment.js
//
function CalendarScript(callback){
	function FullCalendarScript(){
		if(!$.fn.fullCalendar){
	...
	if (!$.fn.moment){
		$.getScript('plugins/moment/moment.min.js', FullCalendarScript);
	}
	else {
		FullCalendarScript();
	}
...
//
// Dynamically   OpenStreetMap Plugin
// homepage: http://openlayers.org
//
function OpenLayersScript(callback){
	if (!$.fn.OpenLayers){
...
//
//  Dynamically   jQuery Timepicker plugin
//  homepage: http://trentrichardson.com/examples/timepicker/
//
function TimePickerScript(callback){
	if (!$.fn.timepicker){
...
//
//  Dynamically  Bootstrap Validator Plugin
//  homepage: https://github.com/nghuuphuoc/bootstrapvalidator
//
function BootstrapValidatorScript(callback){
	if (!$.fn.bootstrapValidator){
		$.getScript('resources/js/user-page/plugins/bootstrapvalidator/bootstrapValidator.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
//
//  Dynamically  jQuery Select2 plugin
...
//
//  Dynamically  Widen FineUper
//  homepage: https://github.com/Widen/fine-uper  v4.3.1 license - GPL3
//
function FineUper(callback){
	if (!$.fn.fineuper){
		$.getScript('plugins/fineuper/jquery.fineuper-4.3.1.min.js', callback);
	}
	else {
		if (callback && typeof(callback) === "function") {
			callback();
...
//
//  Dynamically  Flot plugin
//  homepage: http://www.flotcharts.org  v0.8.2 license- MIT
//
function FlotScripts(callback){
	function FlotScript(){
		$.getScript('plugins/flot/jquery.flot.js', FlotResizeScript);
	}
	function FlotResizeScript(){
		$.getScript('plugins/flot/jquery.flot.resize.js', FlotTimeScript);
	}
	function FlotTimeScript(){
...
function MorrisScripts(callback){
	...
	if (!$.fn.raphael){
		$.getScript('plugins/raphael/raphael-min.js', MorrisScript);
	}
	else {
		MorrisScript();
	}
...
//
//  Dynamically  Fancybox 2 plugin
//  homepage: http://fancyapps.com/fancybox/ v2.1.5 License - MIT
//
function FancyboxScript(callback){
	if (!$.fn.fancybox){
...
---------------------------------------------*/
//
//  Function for  content from url and put in $('.ajax-content') block
//
...
function TestTable3(){
	$('#datatable-3').dataTable( {
		...
		"oTableTools": {
			"sSwfPath": "resources/js/user-page/plugins/datatables/copy_csv_xls_pdf.swf",
			"aButtons": [
				"copy",
				{
					"sExtends":    "collection",
...
/*-------------------------------------------
	Function for File up page (form_file_uper.html)
---------------------------------------------*/
function FileUp(){
	$('#bootstrapped-fine-uper').fineUper({
...
---------------------------------------------*/
//
//  GeoIP JSON data and draw 3 maps
//
function TestMap(){
...
---------------------------------------------*/
//
//  data from Flicks, parse and create gallery
//
...
//
//  scripts and draw Calendar
//
function DrawFullCalendar(){
	CalendarScript(DrawCalendar);
}
//////////////////////////////////////////////////////
//////////////////////////////////////////////////////
//
